...
async function requireOwner(req) {
  if (!url || !serviceKey) return { error: "Server not configured: SUPABASE_SERVICE_ROLE_KEY missing", status: 500 };
...
export async function POST(req) {
  ...
  if (error) {
    const msg = /already/i.test(error.message) ? "That username is already taken" : error.message;
