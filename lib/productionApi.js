...
async function recalcPayPeriod(dateStr) {
  ...
  const record = {
    ...
    total_soil_samples: pay.totalSoilSamples,
    hydroseed_pay: pay.hydroseedPay,
    skid_steer_pay: pay.skidSteerPay,
    soil_sample_pay: pay.soilSamplePay,
...
export async function fetchSeasonTotals(year) {
  ...
  try {
    ...
    const totals = {
      ...
      totalSoilSamples: data.reduce((s, p) => s + (parseInt(p.total_soil_samples) || 0), 0),
      ...
      totalSoilSamplePay: data.reduce((s, p) => s + (parseFloat(p.soil_sample_pay) || 0), 0),
