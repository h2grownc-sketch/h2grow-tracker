...
// Sub-type for hydroseeding jobs (shown when serviceType === "Hydroseeding")
...
// Parse a user-typed money value ("$3,500", "3,500", "3500") to a number.
// Free-text entry with "$" or commas previously produced NaN or wrong totals.
...
export function getAlertMsg(job) {
  ...
  if (job.checks?.jobComplete && !job.sprayDate)
    return "Completion date missing — follow-up alerts off";
  ...
  if (job.checks?.jobComplete && job.sprayDate && !isHydro(job.serviceType)) {
    ...
    if (d > 3 && !job.checks?.followUp3) return "3-day follow-up overdue";
  }
  if (
